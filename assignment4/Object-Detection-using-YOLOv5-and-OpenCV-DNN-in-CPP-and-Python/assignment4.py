...
def post_process(frame, outputs):
	...
	for i in indices:
		...
		width = box[2]
		height = box[3]
		cv2.rectangle(frame, (left, top), (left + width, top + height), BLUE, 3*THICKNESS)
...
if __name__ == '__main__':
    ...
    modelWeights = "models/yolov5n.onnx"            ###Nano
    ...
    while True:
        ...
        #Resize does not really improve fps

        # _INPUT_WIDTH = 320  
        # _INPUT_HEIGHT = 240  
        # frame = cv2.resize(frame, (_INPUT_WIDTH, _INPUT_HEIGHT))

        # Process frame.
        detections = pre_process(frame, net)
        frame = post_process(frame.copy(), detections)
        ...
        fps = 1 / (end - start)
        print(fps)
        cv2.putText(frame, f'FPS: {round(fps, 2)}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
